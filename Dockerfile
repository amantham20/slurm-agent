...
# ============================================================================
FROM public.ecr.aws/docker/library/golang:1.26-bookworm AS gosu-builder

ARG GOSU_VERSION
ARG TARGETOS
ARG TARGETARCH

# Extra CA trust anchors (e.g. corporate TLS-inspecting proxies).
COPY ca-bundle/ /usr/local/share/ca-certificates/extra/
...
# Extra CA trust anchors (e.g. corporate TLS-inspecting proxies).
# ca-bundle/ is empty by default; drop *.crt files there if needed.
COPY ca-bundle/ /etc/pki/ca-trust/source/anchors/
RUN update-ca-trust
...
# ============================================================================
FROM ${BUILDER_BASE} AS lmod-builder
...
# ============================================================================
FROM ${BUILDER_BASE} AS spack-builder
